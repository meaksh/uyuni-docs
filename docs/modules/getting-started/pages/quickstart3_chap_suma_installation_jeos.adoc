[[sles.installation.within.kvm.jeos]]
= JeOS Installation
include::_common-documents/entities.adoc[]
...
. {slsa} by default uses the BTRFS file system.
A mount point is created automatically for [path]``/var/lib/pgsql/`` (even when not installed).
This must be removed or commented out from the [path]``/etc/fstab`` entries.
As root, edit [path]``/etc/fstab`` and comment out or remove the line:
+
...
[WARNING]
.Remove [command]``pgsql`` from the fstab Configuration File
====
If you do not remove this line from fstab the first time you shut down the server you will lose your database.
This occurs because you will have duplicate entries in the fstab file. _Updated tools shipped with recent SPs will no longer require human intervention._
====
+
